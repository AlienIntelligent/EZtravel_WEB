...
const STATUS_CONFIG = {
  ...
  CANCELLED: { label: "Đã hủy",        icon: XCircle,      className: "text-slate-600  bg-slate-50  border-slate-200"  },
};

function StatusBadge({ status }) {
  const key = String(status ?? "").toUpperCase();
  const config = STATUS_CONFIG[key] ?? { label: key || "Không rõ", icon: Clock, className: "text-slate-600 bg-slate-50 border-slate-200" };
