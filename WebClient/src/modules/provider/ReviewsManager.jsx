...
function StarRating({ value }) {
 ...
 <span className="ml-1.5 text-sm font-bold text-foreground">{Number(value ?? 0).toFixed(1)}</span>
 </div>
 );
}
...
function ReplyForm({ reviewId, existingReply, onDone }) {
 ...
 return (
 <form className="mt-3 flex flex-col gap-2 border-t border-border pt-3" onSubmit={handleSubmit}>
 <label className="text-xs font-semibold text-muted-foreground" htmlFor={`reply-${reviewId}`}>
 {existingReply ? "Chỉnh sửa phản hồi" : "Phản hồi khách hàng"}
 </label>
 <textarea
 id={`reply-${reviewId}`}
 className="min-h-20 w-full resize-y rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground outline-none transition focus:border-primary focus:bg-background focus:ring-2 focus:ring-primary/10"
...
function ReviewCard({ review }) {
 ...
 return (
 <article className="space-y-1 rounded-md border border-border bg-background p-4 shadow-sm transition hover:shadow-md sm:p-5">
 {/* Header */}
 ...
 className="h-14 w-14 rounded-md border object-cover" onError={(e) => { e.target.onerror = null; e.target.src = "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?q=80&w=600&auto=format&fit=crop"; }} />
 ))}
 </div>
 ) : null}
 ...
 {hasReply && (
 <div className="rounded-md border-l-4 border-primary/40 bg-primary/5 px-4 py-2.5 text-sm text-muted-foreground">
