          ...
          <div className="space-y-2">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-xs p-3 bg-muted/30 rounded-xl border border-border/50">
              {rules.map((rule, idx) => {
                ...
                return (
                  <div key={idx} className={`flex items-center gap-1.5 transition-colors duration-200 ${isValid ? 'text-success' : 'text-slate-600 dark:text-slate-400 font-medium'}`}>
                    {isValid ? <CheckCircle2 className="w-16 h-16 text-success" /> : <XCircle className="w-16 h-16 text-slate-400 dark:text-slate-500" />}
