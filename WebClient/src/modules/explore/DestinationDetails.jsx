...
const getDestinationName = (destination) => destination?.name ?? destination?.tenDiaDiem ?? "Địa điểm";
...
export default function DestinationDetails() {
 ...
 {image ? (
 <img src={image} alt={getDestinationName(destination)} className="h-72 w-full object-cover sm:h-96" onError={(e) => { e.target.onerror = null; e.target.src = "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?q=80&w=600&auto=format&fit=crop"; }} />
 ) : (
 ...
 {destination.luotXem ?? destination.viewCount ?? 0} lượt xem
 </span>
 </div>
 ...
 {getDescription(destination) || "Thông tin chi tiết đang được cập nhật."}
 </p>
 </main>

 <aside className="h-fit rounded-md border bg-background p-5">
 <h2 className="text-sm font-bold text-foreground">Thông tin địa điểm</h2>
 <dl className="mt-4 space-y-4 text-sm">
 <div>
 <dt className="text-muted-foreground">Tỉnh thành</dt>
 <dd className="mt-1 font-semibold text-foreground">{destination.tenTinhThanh ?? "Chưa cập nhật"}</dd>
 </div>
 {hasCoordinates && (
 <div>
 ...
 <div>
 <h2 className="text-2xl font-bold text-foreground">Dịch vụ tại địa điểm</h2>
 <p className="mt-1 text-sm text-muted-foreground">Khách sạn, ẩm thực, hoạt động và di chuyển đang hoạt động.</p>
 </div>
 ...
 Chưa có dịch vụ đang hoạt động tại địa điểm này.
 </div>
 ) : (
 ...
 >
 {serviceImage ? (
 <img src={serviceImage} alt={service.name ?? service.tenDichVu} className="h-44 w-full object-cover" onError={(e) => { e.target.onerror = null; e.target.src = "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?q=80&w=600&auto=format&fit=crop"; }} />
 ) : (
