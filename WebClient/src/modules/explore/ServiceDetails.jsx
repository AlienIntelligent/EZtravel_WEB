...
const getServiceName = (service) => service?.name ?? service?.tenDichVu ?? "Dich vu";
...
export default function ServiceDetails() {
  ...
  const handleSubmitReview = async (event) => {
    ...
    try {
      ...
      setFormError("");
      toast({
        title: "Da gui danh gia",
        description: "Danh gia cua ban da duoc luu vao dich vu.",
        variant: "success",
      });
    } catch (err) {
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-8 pb-14">
      ...
      >
        <ArrowLeft className="h-4 w-4" />
        {placeId ? "Quay lai dia diem" : "Quay lai kham pha"}
      </Link>

      <div className="grid gap-3 lg:grid-cols-[1fr_180px]">
        <div className="overflow-hidden rounded-md border bg-muted">
          {selectedImage ? (
            <img src={selectedImage} alt={getServiceName(service)} className="h-72 w-full object-cover sm:h-96" />
          ) : (
        ...
        {images.length > 1 && (
          <div className="grid grid-cols-3 gap-2 lg:grid-cols-1">
            {images.slice(0, 4).map((image, index) => (
              ...
              >
                <img src={image} alt="" className="h-20 w-full object-cover lg:h-[90px]" />
              </button>
            ))}
          </div>
        )}
        ...
        <main>
          <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            <span className="rounded border bg-muted/40 px-2.5 py-1 text-xs font-semibold text-foreground">
              {serviceTypeLabels[serviceType] ?? serviceType ?? "Dich vu"}
            </span>
          ...
          <div className="mt-7 border-t pt-6">
            <h2 className="text-xl font-bold text-foreground">Mo ta dich vu</h2>
      ...
      <section className="border-t pt-8">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-2xl font-bold text-foreground">Danh gia dich vu</h2>
          ...
          <div>
            {user ? (
              ...
              <div className="rounded-md border bg-background p-5 text-sm text-muted-foreground">
                ...
                <p>Ban can dang nhap de gui danh gia.</p>
          ...
          <div className="space-y-4">
            {isLoadingReviews ? (
              ...
              <div className="rounded-md border bg-background px-4 py-10 text-center text-sm text-muted-foreground">
                Chua co danh gia nao cho dich vu nay.
              </div>
            ) : (
              reviews.map((review) => {
                ...
                return (
                  <article key={review.id ?? review.maDanhGia} className="rounded-md border bg-background p-5">
                    ...
                    {review.images?.[0] && (
                      <img src={review.images[0]} alt="Anh danh gia" className="mt-4 h-32 rounded-md object-cover" />
                    )}
                    {review.reply && (
