...
const statusConfig = {
 DRAFT: { Icon: Lock, label: "Bản nháp", className: "bg-muted text-muted-foreground" },
 ...
 IN_PROGRESS: { Icon: CheckCircle2, label: "Đang diễn ra", className: "bg-blue-100 text-blue-800" },
 COMPLETED: { Icon: CheckCircle2, label: "Đã hoàn thành", className: "bg-emerald-100 text-emerald-800" },
};
...
export default function TopPlannerBar({
 ...
 return (
 <header className="flex min-h-14 shrink-0 items-center justify-between gap-3 border-b border-border bg-background px-4 py-2 shadow-sm sm:px-5">
 <div className="min-w-0">
 <h1 className="max-w-[210px] truncate text-base font-bold text-foreground sm:max-w-md sm:text-lg">
 {tripName}
 </h1>
 ...
 <button 
   onClick={onShare}
   className="inline-flex items-center justify-center rounded-md border border-input bg-background px-3 py-1.5 text-xs font-semibold hover:bg-accent hover:text-accent-foreground"
 >
   Chia sẻ
 </button>
 ...
 {status !== 'IN_PROGRESS' && status !== 'COMPLETED' && (
   <button 
     onClick={onStartTrip}
     className="inline-flex items-center justify-center rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90 shadow-sm"
   >
     Bắt đầu
   </button>
 )}
