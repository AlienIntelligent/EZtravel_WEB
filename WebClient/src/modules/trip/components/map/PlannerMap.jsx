...
export default function PlannerMap() {
 ...
 return result;
  ...
  if (!apiKey || apiKey === "DUMMY_API_KEY") {
    return <PlannerMapFallback markers={markers} />;
  }
  return <PlannerMapGoogle apiKey={apiKey} markers={markers} />;
}

function PlannerMapFallback({ markers }) {
 return (
 <section className="h-full min-h-[360px] overflow-y-auto bg-background">
 <header className="flex items-center gap-2 border-b border-border px-4 py-3">
 <MapPinned className="h-4 w-4 text-primary" aria-hidden="true" />
 <h2 className="text-sm font-bold text-foreground">Điểm đến trong lịch trình</h2>
 </header>
 {markers.length === 0 ? (
 <div className="flex min-h-72 flex-col items-center justify-center px-6 text-center">
 <MapPin className="h-8 w-8 text-slate-300" aria-hidden="true" />
 <p className="mt-3 text-sm text-muted-foreground">Chưa có địa điểm trên bản đồ.</p>
 </div>
 ) : (
 ...
 <span className="min-w-0 truncate text-sm font-semibold text-foreground">{marker.name}</span>
 </div>
 ))}
 </div>
 )}
...
function PlannerMapGoogle({ apiKey, markers }) {
 ...
 <span className="h-8 w-8 animate-spin rounded-full border-2 border-border border-b-primary" />
