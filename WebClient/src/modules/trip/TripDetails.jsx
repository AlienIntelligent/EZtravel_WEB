...
import { Edit3, Home, ChevronRight, Loader2, MessageCircle, RefreshCw, Send } from "lucide-react";
          ...
          {comments.map((comment) => (
            <article key={comment.id} className="td-comment">
              {comment.authorAvatar ? (
                <img src={comment.authorAvatar} alt={comment.authorName} className="td-comment__avatar" />
              ) : (
      ...
      >
        ...
        <div className="td-hero__content">
          <nav className="td-breadcrumb">
            ...
            <Link to="/trips">Lộ trình của tôi</Link>
            <ChevronRight size={14} />
            <span>{trip.tenLichTrinh}</span>
          </nav>
          <h1 className="td-hero__title">{trip.tenLichTrinh}</h1>
