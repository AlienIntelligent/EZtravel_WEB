...
const getNotificationTitle = (notification) =>
  notification.title ?? notification.tieuDe ?? "Thong bao";
  ...
  return (
    <div className="container mx-auto w-full max-w-5xl px-4 py-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Thong bao</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {unreadNotifications.length} thong bao chua doc trong {notifications.length} thong bao.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="flex min-h-[260px] items-center justify-center rounded-md border bg-background">
          <div className="text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">Dang tai thong bao...</p>
          </div>
        </div>
      ) : isError ? (
        <div className="rounded-md border border-red-200 bg-red-50 px-4 py-4 text-sm text-red-700">
          Khong the tai thong bao. Vui long thu lai.
        </div>
        ...
        <div className="flex min-h-[260px] flex-col items-center justify-center rounded-md border bg-background px-4 text-center">
          ...
          <h2 className="mt-4 text-lg font-semibold text-foreground">Chua co thong bao</h2>
          <p className="mt-1 text-sm text-muted-foreground">
